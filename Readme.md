...
Other data is used to analyze the previous ones:
 ...
 - Terrain: Numerical Terrain Model to have the elevation of each
   measurement (IGN - BDAlti data - 25 m horizontal resolution)
...
### Code
Several functions have been developped in order to download and analyze the 
VeloClimat data. The main script (data_processing) calls functions to do 
the full processing of the data (both for VeloClimameter or VeloClim'App):
 1. download: To download (and store) the data,
 2. cleanAndFormat: To clean the data (remove duplicate points) and gather 
