...
Some data directly derive from the project:
    - VeloClim_App: Air temperature observed during 
    collaborative mapping events
...
the effect of an other:
    ...
    - SA1. Keeping values only for wind speed higher
    than a certain threshold
    - SA2. Keeping values only when there is a minimum
    number of seconds spent in a same LCZ
...
3. gather: 
    a. VeloClimameter: with physiological record with RSU indicators and
elevation
    b. VeloClim'App: with RSU indicators and elevation and calculate wind speed
4. createRef: To create a reference air temperature based on a bilinear
 interpolation of the MF stations and an altitude correction for day-time
 VeloClimameter data.
5. tempDifferences: Calculate air temperature differences from the reference
6. investigations: investigate the assumptions described above in the order 
